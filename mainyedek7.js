...
navigator.mediaDevices.getUserMedia({ video: true })
  .then(stream => {
    // Kullanıcı izin verdi → webcam aktif
    ...
    // İnci webcam görüntüsünü kullanıyor
   material.map = videoTexture;
   material.needsUpdate = true;
  })
  .catch(err => {
    console.warn("Kullanıcı kamera izni vermedi:", err);

    // Kullanıcı izin vermedi → inci orijinal rengine dönüyor
material.map = null;
material.color = new THREE.Color(0xf5f2e8);
...
// SKETCH PNG — 3D İNCİNİN ÜSTÜNE BİNDİRME
const loader = new THREE.TextureLoader();
loader.load('sketch.png', (texture) => {
  ...
  sprite.scale.set(10,10, 1); // boyut
  sprite.position.set(0, 0.9, 0); // inci üstüne hizalama
